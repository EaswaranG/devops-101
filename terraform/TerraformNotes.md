...
- **variables.tf:** Contains declarations for input variables used in your Terraform configuration. This file defines variable names, types, and optional descriptions, but does not assign values.
- **terraform.tfvars:** Stores the actual values for the variables declared in `variables.tf`. This file is used to provide input values when running Terraform commands.
...
Terraform loads variables from multiple sources, applying them in a specific order. Later sources override earlier ones. If none of the variable source is defined, it falls back to default in variables.tf. If the default is also not mentioned, terraform is going to request for the source in the CLI as input. Note: This could break the automation if any. 

The precedence of terraform variable sources are as below (Higher the number, higher the precedence)
...
4. **Any `*.auto.tfvars` or `*.auto.tfvars.json` files**  
   These are loaded in lexical order by filename.
...
Any `-var` and `-var-file` options on the command line, in the order they are provided. This includes variables set by an HCP Terraform workspace.
   Example:  
   ```bash
   terraform apply -var="region=ap-south-1" -var-file="terraform.dev.tfvars"
   ```
**Note:**  
If the same variable is defined in multiple places, the value from the source with the highest precedence is used.

### Variable Validation

- Variable Validation in terraform is used to specify custom validation rules how and when the variable can be used.
Example:
